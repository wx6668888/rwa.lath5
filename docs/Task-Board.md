...
**当前代码基线：** Next.js PWA 高保真原型与 NestJS API 已在正式仓库；PostgreSQL 迁移、身份/账户数据模型、邮箱/OAuth/钱包签名身份闭环已实现。真实合作方、资金、订单、KYC 与 Polymarket 接入尚未开始，不得按生产能力宣称。
...
| API-002 | 身份与账户：用户、邮箱、OAuth、钱包签名挑战 | 待审核 | Hermes | API-001、DB-001 | nonce、会话、绑定冲突与恢复均有测试 |
| API-003 | 会话、双重验证、Passkey、设备与安全事件 | 进行中 | Codex | API-002 | 设备信任、会话撤销、敏感操作重验和审计生效 |
| API-004 | KYC、资格、地域、制裁与风险规则服务 | 待审核 | Hermes | API-002、DB-002、PARTNER-001 | 用户、资产和订单均由同一资格服务判定；KYC/制裁/地域/风险规则与统一资格评估已实现，单元验证通过。详见 `docs/task-notes/API-004.md` |
...
| DB-002 | 用户、身份、会话、设备、KYC、资格、风险和审计模型 | 待审核 | Codex | DB-001 | 11 个核心表、约束/索引、不可变审计、共享状态词典与密文/HMAC 策略已通过 PostgreSQL 集成测试；见 `docs/task-notes/DB-002.md` |
...
- **已实现、等待产品审核：** `FE-002`、`FE-004`、`API-001`、`API-002`、`DB-001`、`DB-002`、`API-004`。它们均有对应任务备注和代码/测试证据；按产品负责人规则，未获得明确确认前一律保持“待审核”。
